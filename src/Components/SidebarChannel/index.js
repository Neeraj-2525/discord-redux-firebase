...
const SidebarChannel = ({ id, channelName, channelType }) => {
  ...
  return (
    <div
      className={ `sidebar-channel cur-po ${isActive? 'active-channel':''}` }
      onClick={() =>
        dispatch(
          setChannelInfo({
            channelId: id,
    ...
    >
      <h4>
        <span className='sidebar-channel-hash cntr'>
          {channelType === "voice" ? channelIcon[1].speaker : channelIcon[0].hash}
        </span>
        {channelType === "voice" ? 'General' : channelName}
        {/* {channelName} */}
      </h4>
      <div className={ `setting-icon ${isActive? 'active-icon':''}` }>
        {channelIcon[2].setting}
      </div>
    </div>
  )
}
